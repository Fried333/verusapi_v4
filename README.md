...
|---|---|
| `/verus/market` | Native market data for the Verus Explorer — all converters, pairs (including basket token pairs), currencies with USD prices. Unlike aggregator endpoints, basket token pairs are not filtered out. |
...
On first run (or when new baskets are created on-chain), generate the converter list:

```bash
python3 converter_discovery.py
```

This creates `converter_discovery.json` with all active converters. Re-run when new baskets appear.
